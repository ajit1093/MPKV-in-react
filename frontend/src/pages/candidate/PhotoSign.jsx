...
/**
 * Upload Photograph and Signature — exact match to UploadPhotoAndSign.aspx
 *
 * Key behaviours (mirrors legacy exactly):
 *  1. GET  /api/applicationform/photo-sign  → load existing photo + sign URLs from DB
 *  2. POST /api/applicationform/upload-photo  (multipart) → upload photo, save URL, reload
 *  3. POST /api/applicationform/upload-sign   (multipart) → upload sign, save URL, reload
 *  4. POST /api/applicationform/photo-sign/save → mark step done → navigate /candidate/documents
 *  5. Proceed button DISABLED until both photo + sign are uploaded (mirrors btnProceed.Enabled)
 *  6. Photo preview: 80px × 100px  (legacy: width:80 height:100)
 *  7. Sign  preview: 130px × 55px  (legacy: width:130 height:55 in the sign div)
 *  8. After photo upload → update AuthContext photoPath → navbar circular pic refreshes
 *  9. Image URLs come from API as full Azure Blob URLs (https://...) or relative /uploads/...
 *     → display as-is when starts with http, else prefix with empty string (served via API proxy)
 *
 * Validation (client-side mirrors CheckPhoto / CheckSign JS):
 *  Photo: JPG/JPEG only, 10 KB – 100 KB
 *  Sign:  JPG/JPEG only,  5 KB –  50 KB
 */
export default function PhotoSign() {
  ...
  // Reset img-error flags whenever URLs change (new upload or page load)
  useEffect(() => { setPhotoImgError(false) }, [photoURL])
  useEffect(() => { setSignImgError(false)  }, [signURL])

  const photoInputRef = useRef(null)
  const signInputRef  = useRef(null)

  // Proceed enabled only when both are uploaded (mirrors btnProceed.Enabled)
  const bothUploaded = photoURL.length > 0 && signURL.length > 0

  // ── Build display URL ─────────────────────────────────────────────────────
  // The backend now strips ViewFile.aspx wrappers before returning URLs.
  // This client-side helper is a safety net for anything still in localStorage.
  //
  // Handles 3 URL formats:
  //  1. Clean Azure Blob https:// → use directly
  //  2. Legacy ViewFile.aspx?...FileURL=https://... → extract FileURL (handles double-wrapping)
  //  3. Local /uploads/... → prefix with backend origin
  const BACKEND = window.location.port === '5173' ? 'http://localhost:7001' : ''

  const displayUrl = url => {
    if (!url) return ''
    // Unwrap all ViewFile.aspx layers
    let current = url
  ...
  useEffect(() => {
    ...
    applicationFormApi.getPhotoSign()
      .then(res => {
        ...
        setPhotoURL(pUrl)
        setSignURL(sUrl)
        // Resolve the clean URL (strips legacy ViewFile.aspx wrapper if present)
        // and sync to AuthContext so navbar shows the photo immediately
        if (pUrl && updateUser) updateUser({ photoPath: displayUrl(pUrl) })
      })
  ...
  const validatePhoto = file => {
    ...
    if (ext !== 'jpg' && ext !== 'jpeg')
      return 'Photograph Format should be jpg/jpeg.'
  ...
  const validateSign = file => {
    const ext = file.name.split('.').pop().toLowerCase()
    if (ext !== 'jpg' && ext !== 'jpeg')
      return 'Signature Format should be jpg/jpeg.'
  ...
  const handleUploadPhoto = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setPhotoURL(url)
        setPhotoSuccess(res.data.message ?? 'Photograph Uploaded Successfully.')
        setPhotoFile(null)
        if (photoInputRef.current) photoInputRef.current.value = ''
        // Store resolved clean URL in AuthContext → navbar updates immediately
        if (updateUser) updateUser({ photoPath: displayUrl(url) })
      } else {
  ...
  const handleUploadSign = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        const url = res.data.uploadedURL ?? ''
        setSignURL(url)
        setSignSuccess(res.data.message ?? 'Signature Uploaded Successfully.')
        setSignFile(null)
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        navigate('/candidate/documents')   // mirrors Response.Redirect("../Candidate/ApplicationFormSummary.aspx" → documents for now)
      } else {
  ...
  const V = {
    ...
    danger:      '#ef4444',
    bg:          '#f5f6fa',
    // Amber for inst-box — exactly as legacy .inst-box
    amber:       '#fffbeb',
    amberBorder: '#fde68a',
    amberText:   '#92400e',
    amberTitle:  '#b45309',
  ...
  // Step bar config — Documents Upload is active (matches legacy step-bar)
  const steps = [
  ...
  return (
    <div style={{ fontFamily: 'inherit', background: V.bg, minHeight: '100vh', paddingBottom: 40 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12, color: V.textSecond, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
          Application ID
        </span>
        <span style={{ fontSize: 15, fontWeight: 700, color: V.primary }}>
          {applicationId || '—'}
        </span>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '20px 24px 0', flexWrap: 'wrap' }}>
        {steps.map((s, i, arr) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {i < arr.length - 1 && (
              <span style={{ color: V.textLight, fontSize: 12 }}>›</span>
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '20px 24px 24px' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            padding: '12px 24px', background: V.tealLight,
            borderBottom: `1px solid ${V.border}`
          }}>
            <span style={{ width: 3, height: 14, background: V.primary, borderRadius: 2, display: 'inline-block', flexShrink: 0 }} />
            <i className="fas fa-user-circle" style={{ color: V.teal, fontSize: 13 }} />
            <span style={{ fontSize: 12, fontWeight: 700, color: V.teal, textTransform: 'uppercase', letterSpacing: '.06em' }}>
              Upload Photograph
            </span>
          </div>
          ...
          <div style={{ padding: '20px 24px' }}>
            ...
            {/* ── inst-box — Photograph instructions (amber, exact match to legacy) */}
            <div style={{
              background: V.amber, border: `1px solid ${V.amberBorder}`,
              borderRadius: 10, padding: '16px 20px', marginBottom: 20
            }}>
              <div style={{
                fontSize: 12, fontWeight: 700, color: V.amberTitle,
                textTransform: 'uppercase', letterSpacing: '.06em',
                marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6
              }}>
                <i className="fas fa-exclamation-circle" /> Instructions
              </div>
              <ol style={{ margin: 0, paddingLeft: 20, color: V.amberText, fontSize: 13, lineHeight: 1.8 }}>
                ...
                <li>Preferred photograph size should be between <strong>10KB and 100KB</strong>. Dimensions should be Width: <strong>139px</strong>, Height: <strong>178px</strong>.</li>
              ...
              <div style={{ color: V.amberWarn, fontWeight: 600, fontSize: 13, marginTop: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                Please upload the correct photograph, otherwise your application will be cancelled.
              </div>
            </div>
            ...
            {/* upload-row — photo preview (80×100) + controls */}
            <div style={{
              display: 'flex', alignItems: 'center', gap: 20,
              background: V.tealLight, border: `1.5px dashed ${V.tealBorder}`,
              borderRadius: 12, padding: '16px 20px', flexWrap: 'wrap'
            }}>
              {/* preview — 80×100 matches legacy divPhoto style="width:80px;height:100px" */}
              <div style={{
                width: 80, height: 100,
                border: `1.5px solid ${photoURL && !photoImgError ? V.primary : V.border}`,
                ...
                {photoURL && !photoImgError ? (
                  ...
                  <div style={{
                    ...
                    <i className="fas fa-user" style={{ fontSize: 28, color: V.textLight }} />
                    <span style={{ fontSize: 9, color: V.textLight, textAlign: 'center', lineHeight: 1.3 }}>No Photo</span>
                  </div>
                )}
              </div>
              ...
              <div style={{ flex: 1, minWidth: 240 }}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                  <input
                    ...
                    style={{
                      fontSize: 12.5, border: `1px solid ${V.border}`,
                      borderRadius: 6, padding: '6px 10px', background: '#fff',
                  ...
                  <button
                    ...
                    style={{
                      background: (!photoFile || uploadingPhoto) ? '#e2e8f0' : V.primary,
                      color: (!photoFile || uploadingPhoto) ? '#6b7280' : '#fff',
                ...
                <p style={{ fontSize: 11, color: V.textLight, margin: '6px 0 0' }}>
                  <i className="fas fa-info-circle" /> JPG/JPEG only &nbsp;•&nbsp; 10 KB – 100 KB &nbsp;•&nbsp; 139 × 178 px recommended
                </p>

                {photoError && (
                  <p style={{ fontSize: 12, color: V.danger, margin: '6px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <i className="fas fa-exclamation-circle" /> {photoError}
                  </p>
                )}
                {photoSuccess && (
                  <p style={{ fontSize: 12, color: V.primary, margin: '6px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <i className="fas fa-check-circle" /> {photoSuccess}
                  </p>
                )}
              </div>
            </div>
          ...
          <div style={{
            ...
            <span style={{ width: 3, height: 14, background: V.primary, borderRadius: 2, display: 'inline-block', flexShrink: 0 }} />
            <i className="fas fa-signature" style={{ color: V.teal, fontSize: 13 }} />
            <span style={{ fontSize: 12, fontWeight: 700, color: V.teal, textTransform: 'uppercase', letterSpacing: '.06em' }}>
              Upload Signature
            </span>
          </div>
          ...
          <div style={{ padding: '20px 24px' }}>
            ...
            {/* ── inst-box — Signature instructions */}
            <div style={{
              background: V.amber, border: `1px solid ${V.amberBorder}`,
              borderRadius: 10, padding: '16px 20px', marginBottom: 20
            }}>
              ...
              <ol style={{ margin: 0, paddingLeft: 20, color: V.amberText, fontSize: 13, lineHeight: 1.8 }}>
                ...
                <li>Preferred signature size should be between <strong>5KB and 50KB</strong>. Dimensions should be Width: <strong>198px</strong>, Height: <strong>60px</strong>.</li>
              </ol>
              <div style={{ color: V.amberWarn, fontWeight: 600, fontSize: 13, marginTop: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                <i className="fas fa-exclamation-triangle" />
                Please upload the correct signature, otherwise your application will be cancelled.
              </div>
            </div>
            ...
            {/* upload-row — sign preview (130×55) matches legacy divSign style="width:130px;height:55px" */}
            <div style={{
              display: 'flex', alignItems: 'center', gap: 20,
              background: V.tealLight, border: `1.5px dashed ${V.tealBorder}`,
              borderRadius: 12, padding: '16px 20px', flexWrap: 'wrap'
              ...
              <div style={{
                width: 130, height: 55,
                border: `1.5px solid ${signURL && !signImgError ? V.primary : V.border}`,
                borderRadius: 8, background: '#f8fafc',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                overflow: 'hidden', flexShrink: 0
              }}>
                {signURL && !signImgError ? (
                  <img
                    src={displayUrl(signURL)}
                    alt="Signature"
                    style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 4 }}
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: 9, color: V.textLight }}>No Signature</span>
                  </div>
                )}
              </div>
              ...
              <div style={{ flex: 1, minWidth: 240 }}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <button
                    ...
                    disabled={!signFile || uploadingSign}
                    style={{
                      background: (!signFile || uploadingSign) ? '#e2e8f0' : V.primary,
                      color: (!signFile || uploadingSign) ? '#6b7280' : '#fff',
                      border: 'none', padding: '9px 20px', borderRadius: 8,
                      fontSize: 13.5, fontWeight: 600,
                      cursor: (!signFile || uploadingSign) ? 'not-allowed' : 'pointer',
                      display: 'flex', alignItems: 'center', gap: 6,
                      whiteSpace: 'nowrap', fontFamily: 'inherit'
                    }}
                ...
                {signError && (
                  <p style={{ fontSize: 12, color: V.danger, margin: '6px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <i className="fas fa-exclamation-circle" /> {signError}
                  </p>
                )}
                {signSuccess && (
                  <p style={{ fontSize: 12, color: V.primary, margin: '6px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <i className="fas fa-check-circle" /> {signSuccess}
                  </p>
                )}
              </div>
            </div>
          ...
          <div style={{
            ...
            borderTop: `1px solid ${V.borderLight}`, background: '#f8fafc',
            borderRadius: '0 0 14px 14px', flexWrap: 'wrap', gap: 12
          }}>
            {/* left — info note */}
            ...
            {/* center — Back + Proceed buttons */}
            <div style={{ display: 'flex', gap: 10, position: 'absolute', left: '50%', transform: 'translateX(-50%)' }}>
              <button
                type="button"
                onClick={() => navigate('/candidate/preferences')}
                style={{
                  background: 'transparent', color: V.textPrimary,
                  border: `1.5px solid ${V.border}`, padding: '9px 20px',
              ...
              {/* Proceed — disabled until both uploaded, mirrors btnProceed.Enabled */}
              <button
                type="button"
                ...
                style={{
                  background: (!bothUploaded || saving) ? '#d1fae5' : V.primary,
                  color: (!bothUploaded || saving) ? '#6b7280' : '#fff',
                  border: 'none', padding: '10px 24px', borderRadius: 8,
                  fontSize: 13.5, fontWeight: 600,
                  cursor: (!bothUploaded || saving) ? 'not-allowed' : 'pointer',
      ...
      <style>{`
        @media (max-width: 768px) {
          .upload-row-inner { flex-direction: column !important; align-items: flex-start !important; }
        }
      `}</style>
    </div>
  )
